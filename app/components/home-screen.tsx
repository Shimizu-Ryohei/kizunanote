    ...
    <article className="flex min-h-[112px] items-center gap-7 rounded-lg bg-white px-7 shadow-[0_1px_0_rgba(0,0,0,0.01)]">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-2 flex min-w-0 flex-wrap items-center gap-3">
          {profile.birthday ? (
            <span className="max-w-[148px] truncate rounded-full bg-[var(--color-sub)] px-3 py-1 text-[9px] font-black tracking-[0] text-[var(--color-main)]">
              {profile.birthday}
            </span>
          ) : null}
          <span className="rounded-full bg-[var(--color-sub)] px-3 py-1 text-[9px] font-black tracking-[0] text-[var(--color-main)]">
            ノート {profile.noteCount}件
          </span>
        </div>
      </div>
    </article>
